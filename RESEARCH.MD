...
Логические связки реализованы через макросы. Они получаются ленивые,
потому что макросы принимают аргументы без вычисления, а внутри аргументы
вычисляются только если нужно.
...
Цикл for реализован через макрос
```lisp
(let for 
	(macro [var coll body] 
	    (do 
